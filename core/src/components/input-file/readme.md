...
| `value`      | `undefined`   | The value of the toggle does not mean if it's checked or not, use the `checked`<br>property for that.<br>The value of a toggle is analogous to the value of a `<input type="checkbox">`,<br>it's only used when the toggle participates in a native `<form>`. | `File \| File[]`                                                                                 | `undefined`   |
