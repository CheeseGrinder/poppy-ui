...
| `value`     | `value`      | The value of the radio does not mean if it's checked or not, use the `checked`<br>property for that.<br>The value of a radio is analogous to the value of a `<input type="radio">`,<br>it's only used when the radio participates in a native `<form>`. | `any`                                                                                 | `''`          |
